...
class bq25792:
    ...
    class REG1B_Charger_Status_0(BQ25795_REGISTER):
        ...
        def __init__(self, addr=0x1B, value = 0):
            super().__init__(addr, value)
            self.IINDPM_STAT         = ((self._value & 0b10000000) >> 7)
            self.VINDPM_STAT         = ((self._value & 0b01000000) >> 6)   
            self.WD_STAT             = ((self._value & 0b00100000) >> 5)
            self.POORSRC_STAT        = ((self._value & 0b00010000) >> 4)
            self.PG_STAT             = ((self._value & 0b00001000) >> 3)
            self.AC2_PRESENT_STAT    = ((self._value & 0b00000100) >> 2)
            self.AC1_PRESENT_STAT    = ((self._value & 0b00000010) >> 1)
            self.VBUS_PRESENT_STAT   = ((self._value & 0b00000001) >> 0)
        def set (self, value):
            super().set(value)
            self.IINDPM_STAT         = ((self._value & 0b10000000) >> 7)
            self.VINDPM_STAT         = ((self._value & 0b01000000) >> 6)   
